...
function ProxyCell(props: { profile: Profile; onChecked: () => void }) {
  ...
  return (
    <div className="space-y-0.5">
      ...
      {c && (
        <p className={`text-[11px] ${c.ok ? 'text-ok' : 'text-danger'}`} title={c.error ?? c.checkedAt}>
          {c.ok ? `✓ 出口 ${c.ip} · ${c.latencyMs}ms` : `✗ ${(c.error ?? '失败').slice(0, 40)}`}
        </p>
      )}
    </div>
  );
    ...
    >
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="col-span-2 mt-2 border-t border-white/5 pt-4">
          <p className="mb-3 text-[13px] font-medium text-slate-300">代理设置（代理几乎都需要认证，已原生支持，§ADR-1）</p>
