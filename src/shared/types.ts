...
export interface ProxyCheckResult {
  ...
  latencyMs?: number;
  error?: string;
  /** WebRTC 候选收集到的 IP（去重）；空 = 未收集到 */
  webrtcIps?: string[];
  /** WebRTC 是否泄露真实 IP（候选里出现 ≠ 出口 IP 的公网地址） */
  webrtcLeak?: boolean;
  checkedAt: string;
}

/** 根据代理出口 IP 生成的指纹建议（时区/语言与 IP 地理位置对齐） */
export interface GeoFingerprintSuggestion {
  ip: string;
  country: string;
  countryCode: string;
  city?: string;
...
export interface Settings {
  ...
  recordTasks: boolean; // 任务执行时录制屏幕（默认关，避免磁盘/性能开销）
