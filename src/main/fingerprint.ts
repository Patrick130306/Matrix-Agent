...
export function buildFingerprintInitScript(profile: Profile): string {
  ...
  const config = {
    ...
    // §6.5 一致性：核心数上限受显卡档次约束（核显 + 32 核是低级穿帮）
    hardwareConcurrency: Math.min(clampInt(profile.hardwareConcurrency, 2, 16, 8), preset.maxCores),
...
(() => {
  ...
  // ---- Stealth：permissions.query 通知权限（自动化环境常返回 denied，真实 Chrome 默认 prompt） ----
  // 注意：这是实例方法劫持（在 Navigator.prototype.permissions 的实例对象上），不能走 defineGetter。
  if (navigator.permissions && navigator.permissions.query) {
    const origQuery = navigator.permissions.query;
    const patchedQuery = nativeWrap(function query(desc) {
      if (desc && desc.name === 'notifications') {
        const state =
          typeof Notification !== 'undefined' && Notification.permission ? Notification.permission : 'prompt';
        return Promise.resolve({ state: state, onchange: null });
      }
      return origQuery.call(this, desc);
    }, 'query');
    try { navigator.permissions.query = patchedQuery; } catch (e) { /* 只读则跳过 */ }
  }

  // ---- Stealth：plugins / mimeTypes（自动化环境常为空数组，真实 Chrome 有 PDF 插件组） ----
  if (navigator.plugins && navigator.plugins.length === 0) {
    ...
    const mkMime = (type) => ({ type: type, suffixes: 'pdf', description: 'Portable Document Format' });
    const pluginArr = [mkPlugin('PDF Viewer'), mkPlugin('Chrome PDF Viewer'), mkPlugin('Chromium PDF Viewer'), mkPlugin('WebKit built-in PDF')];
    pluginArr.item = function (i) { return pluginArr[i] || null; };
    pluginArr.namedItem = function (n) { return pluginArr.find((p) => p.name === n) || null; };
    const mimeArr = [mkMime('application/pdf'), mkMime('text/pdf')];
    mimeArr.item = function (i) { return mimeArr[i] || null; };
    mimeArr.namedItem = function (t) { return mimeArr.find((m) => m.type === t) || null; };
    defineGetter(Navigator.prototype, 'plugins', () => pluginArr);
    defineGetter(Navigator.prototype, 'mimeTypes', () => mimeArr);
  }

  // ---- Stealth：window.chrome 保真（headless / 非 Chrome 内核下缺失时补齐） ----
  if (!window.chrome) {
    try {
  ...
  // ---- Stealth：document.fonts 保真（§6.5 按 OS 注入常见字体,fonts.check 命中返回 true） ----
  // 自动化/容器环境字体稀少,强检测用 fonts.check 枚举字体时一眼穿帮。
  if (window.FontFaceSet && document.fonts) {
    const FONT_NAMES = CFG.fonts;
    const fontSet = {
      size: FONT_NAMES.length,
      check: function (font) {
        if (!font) return true;
        const f = String(font).toLowerCase();
        for (let i = 0; i < FONT_NAMES.length; i++) {
          if (f.indexOf(FONT_NAMES[i].toLowerCase()) >= 0) return true;
        }
        return /(sans-serif|serif|monospace|cursive|fantasy)/.test(f);
      },
      forEach: function (cb) {
        for (let i = 0; i < FONT_NAMES.length; i++) {
          cb({ family: FONT_NAMES[i], status: 'loaded' }, i);
        }
      },
      ...
      [Symbol.iterator]: function () { return FONT_NAMES[Symbol.iterator](); },
    };
