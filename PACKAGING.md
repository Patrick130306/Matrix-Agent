...
NSIS 安装包：`release/Matrix Agent Setup 0.2.0.exe`（版本号以 package.json 为准）。
...
成功标志：日志出现 `building target=nsis` → `artifact created`，`release/` 下生成 `Matrix Agent Setup 0.2.0.exe`。
